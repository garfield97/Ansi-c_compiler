...
%type <node> EXPR_SHIFT EXPR_RELATIONAL EXPR_EQUALITY EXPR_AND EXPR_XOR EXPR_INCLUSIVE_OR 
%type <node> EXPR_LOGIC_AND EXPR_LOGIC_OR EXPR_CONDITIONAL EXPR_ASSIGNMENT OPR_ASSIGNMENT EXPR_CONST DECLARATION SPECIFIER_DECLARATION
%type <node> DECLARATOR_INIT_LIST DECLARATOR_INIT SPECIFIER_STORE_CLASS SPECIFIER_TYPE SPECIFIER_UNION_OR_STRUCT UNION_OR_STRUCT
%type <node> DECLARATION_LIST_STRUCT DECLARATION_STRUCT LIST_SPEC_QUAL LIST_STRUCT_DECLARATOR STRUCT_DECLARATOR SPECIFIER_ENUM
%type <node> LIST_ENUMERATOR ENUMERATOR QUALIFIER_TYPE DECLARATOR DECLARATOR_DIRECT POINTER LIST_QUALIFIER_TYPE LIST_PARAM_TYPE
%type <node> LIST_PARAMETER DECLARATION_PARAMETER LIST_IDENTIFIER NAME_TYPE DECLARATOR_ABSTRACT 
%type <node> DECLARATOR_DIRECT_ABSTRACT INITIALIZER LIST_INITIALIZER DECLARATOR ARG_EXPR_LIST
%type <node> STATEMENT STATEMENT_LABELED STATEMENT_COMPOUND LIST_DECLARATION LIST_STATEMENT STATEMENT_EXPR STATEMENT_SELECTION
%type <node> STATEMENT_ITERATION STATEMENT_JUMP
%type <node> PROGRAM DECLARATION_EXTERNAL DEFINITION_FUNCTION
